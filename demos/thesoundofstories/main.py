...
def load_story_details():
    ...
    local_story_path = construct_path_from_root('raw_data/the_sound_of_stories/story.json')
    if os.path.exists(local_story_path):
        print("Local story.json found. Loading...")
        with open(local_story_path, 'r', encoding='utf-8') as file:
            story_data = json.load(file)
            print("Successfully loaded local story.json.")
            print("First few lines of story.json:")
            print(json.dumps(story_data, indent=4)[:100])  # Print the first 100 characters for brevity
            print("Keys within story.json:")
            print(story_data.keys())  # Print the keys within the JSON data
            return story_data
    else:
        # Fetch story.json from GCS
        gcs_client = storage.Client()
        bucket_name = 'thesoundofstories'
        blob_name = 'berlayar/raw_data/story.json'
        bucket = gcs_client.get_bucket(bucket_name)
        blob = bucket.blob(blob_name)
        story_data = blob.download_as_text()
        print("Story.json fetched from GCS. Loading...")
        story_data_json = json.loads(story_data)
        print("Successfully loaded story.json from GCS.")
        print("First few lines of story.json:")
        print(json.dumps(story_data_json, indent=4)[:100])  # Print the first 100 characters for brevity
        print("Keys within story.json:")
        print(story_data_json.keys())  # Print the keys within the JSON data
...
async def on_begin_storytelling(action):
    ...
    # Print the first line from encounter_0_text
    encounter_0_first_line = encounter_0_text.split('\n')[0]
    print("First line from encounter_0_text:", encounter_0_first_line)
