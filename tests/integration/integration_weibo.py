...
def test_brands_and_retrieve_tweets(self):
    for company, details in ORGANIZATIONS.items():
        for idx, brand_english in enumerate(details["en"]):
            brand_mandarin = details["zho"][idx]

            # Retrieve tweets
            self.weibo.ingest(company, brand_english, "2023-01-01 00:00:00", "2023-01-07 23:59:59")

            # Determine the directory for the JSONL file
            final_directory = os.path.join(self.weibo.base_path, "raw_data", "weibo", company, brand_english)
